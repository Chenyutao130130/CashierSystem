...
namespace CashierSystem
{
    public partial class Huang_System : Form
    {
        ...
        public void LoadAllDgv()
        {
            ...
            for (int i = 0; i <= 10; i++)
            {
                ...
                {  
                    //商品信息 货架
                    LoadGoodsInfo();//加载条件框
                    //与商品管理相比,商品展示 没有商品单价(进货价)显示                    
                    this.dgvGoodsInfo.Tag = false;//false 表示需要更新数据  true 表示不需要更新数据
                    this.dgvGoodsInfo = dataGridViewHelper.Init(this.dgvGoodsInfo, nameList, handerTxt, hideIndex); //控件初始化
                    GetDgv(0);
                }
                else if (i == 1)  
                {  
            ...
            switch (id)
            {
                ...
                case 2:    //商品管理
                    if (!(bool)this.dgvGoodSInfoManager.Tag)
                    {
                        ...
                        dataTable = DataManager.GoodsInfoBLL.GetDataTablebyPammer(searchModel);  //输入搜索条件
                        if (!isExport)
                        {
                ...
                case 6: //商品单位
                    ...
                    if (!isExport)
                    {
                        this.dgvUnitInfo = dataGridViewHelper.FillData(this.dgvUnitInfo, dataTable);  //显示所有商品单位 记录
                        this.tspUnitInfoCount.Text = dataTable.Rows.Count.ToString();   //显示商品单位记 记录条数
                    }
                    else
                    {
                        //保存并导出数据表
        ...
        public void LoadGoodsInfoStatus(SearchModel searchModel)
        {
            ...
            double pageSize = Setting.GoodsInfoPageCount;     //每页保存30
            if (tspLblGoodsPageCount.Tag.ToString() != "-1")
            {
                ...
                int count = DataManager.GoodsInfoBLL.GetDataTableCountByPammer(searchModel2); //总记录数
            ...
            else
            {
                ...
                tspGoodsLastPage.Tag = pageStartIndex[pageNow - 1];   //上1页
                tspGoodsNextPage.Tag = pageStartIndex[pageNow + 1];   //下1页
            }
        ...
        {
            int pageSum = Convert.ToInt32(tspLblGoodsPageCount.Text);   //总页数
            int pageNow = Convert.ToInt32(tspGoodsPage.Text);           //当前页
            int lastId = Convert.ToInt32(tspGoodsLastPage.Tag);         //上1页
            bool isTrue = CommonHelper.GetTruePage(pageSum, pageNow, lastId, false); //上
            if (isTrue)
            {
        ...
        private void tspGoodsNextPage_Click(object sender, EventArgs e)
        {
            ...
            bool isTrue = CommonHelper.GetTruePage(pageSum, pageNow, nextId, true); //下
            if (isTrue)
            {
                myGoodInfoSearchModel.StartIndex = nextId;//一页开始！！！！记录起始偏移
                tspGoodsPage.Text = (pageNow + 1).ToString();
                SearchLoadGoodsInfo(myGoodInfoSearchModel);
        ...
        private void SearchLoadGoodsInfo(SearchModel searchModel = null, int startIndex = 0)
        {
            if (searchModel == null)
            {
                ...
                if (sortId != "0")  //不是全部, 商品类别
                {
                    searchModel.dic.Add("SortId", sortId);
                }
                if (unitId != "0")  //不是全部,  商品单位
                {
        ...
        private void dgvGoodsInfo_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                var dataRow = this.dgvGoodsInfo.SelectedRows[0];     //单品
        ...
        public void LoadOrdersInfo()
        {
            ...
            foreach (OrderInfo order in OrdersInfo)
            {
                ...
                dataGridViewRow.Cells[0].Value = order.GoodsId;   //该种商品id
                dataGridViewRow.Cells[1].Value = order.GoodsName; //该种商品名称
                dataGridViewRow.Cells[2].Value = order.Count;//order.Count;     //该种商品数量
                dataGridViewRow.Cells[3].Value = order.PayPrice;  //该种价格
                dataGridViewRow.Cells[4].Value = order.DisCount;  //折扣
                dataGridViewRow.Cells[5].Value = order.Remark;
                AllMoney += order.PayPrice;                       //共计
                this.dgvOrdersInfo.Rows.Add(dataGridViewRow);
            }

            this.lblOrderMoney.Text = AllMoney.ToString(); //共计
        }
        /// <summary>
        /// 下单
        /// </summary>
        ...
        public bool AddGoodsToOrder(int goodsId, int count = 1)
        {
            ...
            {
                OrderId = DateTime.Now.ToString("yyyyMMddHHmmss");  //下单时间  
            }
            //创建一个订单号  一个列表仅有一个
        ...
        private void dgvOrdersInfo_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                frm_Samll.ShowDialog(this);
                frm_Samll.Focus();
                LoadOrdersInfo();  //关闭修改订单窗体后处理...更新UI
        ...
        public void LoadTodayOrderInfo()
        {
            ...
            DateTime todayEnd = DateTime.Today + new TimeSpan(23, 59, 59);  //结束日期
        ...
        {
               SearchModel  searchModel = new SearchModel();  //创建搜索条件
                ...
                searchModel.dic.Add("TodaySearchMaxMoney", maxMoney.ToString());  //规则
        ...
        public void SearchAllOrderInfo(SearchModel searchModel = null, int startIndex = 0)
        {
            if (searchModel == null)
            {
                searchModel = new SearchModel();
                searchModel.ModelName = "OrderInfo";
                searchModel.PageCount = Setting.AllOrderPageCount;  //30天
                searchModel.StartIndex = startIndex;//开始行
                searchModel.dic = new Dictionary<string, string>();
                var startTime = dateAllOrderStartTime.Value;  //销售日期起始
                var endTime = dateAllOrderEndTime.Value;      //销售日期结束        
        ...
        private void tspEditUnitInfo_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var dataId = Convert.ToInt32(dataRow.Cells[0].Value);//获取选中的记录Id

                UnitInfo unitInfo = DataManager.UnitInfoBLL.GetEntityById(dataId);  //从数据库取

                List<string> tags = new List<string>() { unitInfo.Id.ToString(), unitInfo.UnitName, unitInfo.Remark }; //!!!!!! 用于弹出来的商品单位编辑界面
        ...
        {
            GetDgv(SelectIndex,null,true);  //没有搜索条件
        }
        ...
        private void tspAddSortinfo_Click(object sender, EventArgs e)
        {
            ...
            Frm_SortInfo frm_Samll = Frm_SortInfo.Create();  //创建 商品类别
            frm_Samll.ShowDialog(this);
            frm_Samll.Focus();
            if ((Boolean)frm_Samll.Tag == true)
            {
        ...
        private void tspAddWholeSalerInfo_Click(object sender, EventArgs e)
        {
            ...
            Frm_WholeSalerInfo frm_Samll = Frm_WholeSalerInfo.Create();  //供应商
            frm_Samll.ShowDialog(this);
            frm_Samll.Focus();
            if ((Boolean)frm_Samll.Tag == true)
            {
        ...
        private  void  SaveDataTable(DataTable dataTable)
        {
            try
            {
                ...
                var isSuccess = ExcelHelper.WriteExcel(targetPath, dataTable, true);
                if (isSuccess)
                {
