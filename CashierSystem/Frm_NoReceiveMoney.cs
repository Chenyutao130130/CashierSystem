...
namespace CashierSystem
{
    public partial class Frm_NoReceiveMoney : Form
    {
        ...
        private void btnNoMEnter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (_orderId != null)
                {
                    ...
                    noReceiveMoney.OrderId = _orderId;      //订单号
                    noReceiveMoney.CustomerName = this.txtNoM_CustomerName.Text.Trim();  //客户名称
                    noReceiveMoney.Phone = this.txtNoM_Phone.Text.Trim(); //客户联系电话
                    noReceiveMoney.WaitPayPrice = Convert.ToDecimal(this.txtNoM_Money.Text.Trim()); //客户欠款
                    noReceiveMoney.CreateTime = DateTime.Now;   //创建时间
                    noReceiveMoney.Remark = this.txtNoM_Remark.Text; //备注
                    var isSuccess = DataManager.NoReceiveMoneyBLL.Add(noReceiveMoney);  //写入导数据库
                    if (isSuccess)
                    {
                        ClearData();
                        this.Tag = "true";//通知父窗口 ,可以关闭
        ...
        public void Init()
        {
            ...
            {
                this.txtNoM_OrderId.Text = _orderId;     //订单号
