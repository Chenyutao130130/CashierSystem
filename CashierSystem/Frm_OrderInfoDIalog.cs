...
namespace CashierSystem
{
    public partial class Frm_OrderInfoDIalog : Form
    {
        ...
        public  static string _payPrice = null;  //总价格
         Frm_OrderInfoDIalog()
        {
            InitializeComponent();
        }
        ...
        private void btnOrderOK_Click(object sender, EventArgs e)
        {
            ...
            //获取订单表
            var OrdersInfo = f1.OrdersInfo;             //当前用户下单信息
            ...
            {
                //下订单
                var goodsInfo = DataManager.GoodsInfoBLL.GetEntityById(OrdersInfo[i].GoodsId); //商品信息表
                ...
                var isSucess= DataManager.OrderInfoBLL.Add(OrdersInfo[i]);      //订单表   
                if (!isSucess)
                {
