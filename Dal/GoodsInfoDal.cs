...
namespace Dal
{
    public class GoodsInfoDal : BaseDal<GoodsInfo>
    {
        ...
        public DataTable GetDataTablebyPammer(SearchModel searchModel)
        {
            ...
            {    //商品名称
                string goodsName = "";
                if (dic.TryGetValue("GoodsName", out goodsName))  //字典是否存在GoodsName
                {
                    sql += @" and goods.GoodsName" + " like '%" + goodsName + "%'";  //模糊查找

                }
                
                //价格
                if (dic.TryGetValue("GIM_MixMoney", out string mixMoney)) //字典是否存在GIM_MixMoney
                {
                    sql += @" and goods.PayPrice > " + mixMoney;

                }
                if (dic.TryGetValue("GIM_MaxMoney", out string maxMoney))  //字典是否存在GIM_MaxMoney
                {
            ...
            var dataTable = SqlHelper.GetDataTable(sql);  //返回满足条件的表格
            if (dataTable != null && dataTable.Rows.Count > 0) //存在记录
            {
